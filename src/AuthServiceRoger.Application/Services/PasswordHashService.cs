...
public class PasswordHashService : IPasswordHashService
{
    ...
    public bool VerifyPassword(string password, string hashedPassword)
    {
        try
        {
            ...
            {
                Console.WriteLine("[DEBUG] using Argon2id standard format  verification");
    ...
    private bool VerifyArgon2StandardFormat(string password, string hashedPassword)
    {
        try
        {
            ...
            var exceptedHash = Convert.FromBase64String(FromBase64UrlSafe(hashBase64));
            ...
            var computedHash = argon2Verifier.GetBytes(exceptedHash.Length);

            return computedHash.SequenceEqual(computedHash);
        }
        catch (System.Exception ex)
        {
            Console.WriteLine($"Error verify Argon2 standard format {ex.Message}");
            return false;
        }
    }
